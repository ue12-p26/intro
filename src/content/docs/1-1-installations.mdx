...
<TabItem label="Windows" icon="seti:windows">
<Admonition title="toutes les options par défaut">

à nouveau vous prenez **toutes les options par défaut** (enfin, il faut accepter la licence, évidemment)

**si la question se pose**: ne vous laissez pas distraire par un message qui vous
parlerait d'installer pour d'autres utilisateurs ! par défaut on fait une
installation pour l'utilisateur courant, et c'est suffisant pour nous, une
installation pour tous les utilisateurs n'est pas nécessaire et n'apporte que
des complications...
...
<Admonition title="déplacez dans le dossier Applications !">
...
<TabItem label="Linux" icon="linux">
<Admonition title="Comment utiliser un .deb">

Pour info sur Linux - enfin plus spécifiquement, les distros comme Ubuntu - lorsque vous avez téléchargé un fichier `.deb`, vous pouvez l'installer avec la commande
suivante:
...
<TabItem label="Windows" icon="seti:windows">
  ...
  <Admonition title="download" collapsible>
    ...
    attention: **voyez l'étape suivante pour répondre aux questions**
...
<Admonition title="Pour désactiver les traits orientés IA">

à taper dans un terminal bash:

```bash
curl -k https://intro.info-mines.paris/code/setup-vscode.sh | bash
```
...
* copiez-collez le code ci-dessous dans un fichier qui s'appelle `fact.py`
  ...
  * Control-A ou Control-C ou Control-V: comme d'habitude pour le copier-coller

  et aussi:

  * si vous avez une bonne connection, n'hésitez pas à accepter la suggestion
  qui vous est faite d'installer l'extension Python de Microsoft
...
<Admonition title="on vérifie numpy et jupyter" type="caution" collapsible>
  ...
  <Admonition title="lancer (et sortir de) jupyter" collapsible type="caution">

  on va maintenant le lancer:
...
observez que:
  - votre terminal est à présent occupé (à faire tourner jupyter lab)
  - apprenez à **toujours sortir par le menu *File* -> *Shutdown***
  - et ainsi votre terminal redevient actif
...
git config --global alias.sts "status --short --untracked-files=no"
...
* avec les `alias` on définit des raccourcis; c'est comme ça par exemple qu'on pourra taper simplement  
  `git la`  
